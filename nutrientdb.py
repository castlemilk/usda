...
from usda_to_norm import mapping
...
class NutrientDB:
    ...
    def query_langual(self, ndb_no):
        ...
        for langual in self.database.execute('''
            ...
            thesaurus.append(
                {'code': langual['Factor_Code'], 'description': langual['Description']})
    ...
    def query_langual_foodsource(self, ndb_no):
        ...
        # Get language variants for the food, we only get the languals starting
        # with A,B,C
        for langual in self.database.execute('''
            select * from langual, langdesc where langual.Factor_Code = langdesc.Factor_Code
    ...
    def query_nutrients(self, ndb_no):
        ...
        for nutrient in self.database.execute('''
            ...
            # select * from datsrcln where NDB_No = ? and Nutr_No = ?''', [ndb_no,
            # nutrient['Nutr_No']])]
        ...
        #       }
            ...
            #   # print count
            #   print "------------------------------------------"
            #   print nutrient_filtered
            #   print nutrient['Tagname']
            #   print "------------------------------------------"
            #   break
            # else:
            #   count +=1
    ...
    def insert_row(self, cursor, datatype, fields):
        ...
        # Execute insert
        cursor.execute("insert into " + datatype +
                       " values " + insert_params, fields)
    ...
    def refresh(self, filename, datatype):
        ...
        with open(filename, 'rU') as f:
            for line in f:
                ...
                # We also need to decode the text from the Windows cp1252 encoding used by
                # the USDA files
                fields = [unicode(field.strip().strip('~'), "cp1252")
    ...
    def convert_food_item_list(self):
        """
        Generate food item list containing the mapping between food ID and the
        corresponding meta data line name and food group and source
        """
        food_item_list = {}
        for food in tqdm(self.database.execute('''
                select * from food_des, fd_group where food_des.FdGrp_Cd = fd_group.FdGrp_Cd''')):
            NDB_no = food['NDB_no']
            food_item_list[NDB_no] = {
                                'name': food['Long_desc'],
                                'group': food['FdGrp_Desc'],
                                'source': 'USDA',
                                    }
        with open('usda_food_list.json', 'w') as fd:
            json.dump(food_item_list, fd)
...
def main():
    ...
    parser.add_argument('-p', '--path', dest='path',
                        help='The path to the nutrient data files. (default: data/sr28asc/)', default='data/sr28asc/')
    parser.add_argument('-db', '--database', dest='database',
                        help='The name of the SQLite file to read/write nutrient info. (default: nutrients.db)', default='nutrients.db')
    parser.add_argument('-f', '--force', dest='force', action='store_true',
                        help='Whether to force refresh of database file from flat file. If database file already exits and has some data in it we skip flat file parsing.')
    parser.add_argument('-e', '--export', dest='export', action='store_true',
                        help='Converts nutrient data into json documents and outputs to standard out, each document is seperated by a newline.')
    parser.add_argument('--mhost', dest='mhost',
                        help='Mongo hostname. Defaults to localhost.', default='localhost')
    ...
    parser.add_argument('-l', '--list', dest='list', action='store_true', help='generates food item list and stores as json file', default='usda_food_list.json')
    ...
    if args['export']:
        ...
        nutrients.convert_food_item_list()
